...
public class IPv6TextParser(public val incoming: String) {
    ...
    private fun parseError(cause: String): Either<Nothing, IPAddressParseFailure> {
        return Either.err(IPAddressParseFailure(cause, incoming))
    }
    ...
    private fun checkIncoming(): Either<Unit, IPAddressParseFailure> {
        for (char in incoming) {
            if (char != ':' && char != '[' && char != ']' && !char.isHex()) {
                return parseError("Invalid character: $char")
            }
        }
     ...
     */
    public fun parse(): Either<ByteArray, IPAddressParseFailure> {
        reset()

        val check = checkIncoming()
        @Suppress("UNCHECKED_CAST")
        if (check.isFailure) return check as Either<ByteArray, IPAddressParseFailure>
            ...
            when (val nextChar = iterator.nextChar()) {
                '[' -> {
                    ...
                    if (state != State.STATE_INITIAL) {
                        return parseError("Illegal left bracket")
                    }
                    openBrackets = true
                ...
                ']' -> {
                    ...
                    if (state != State.STATE_HEXDIGIT && state != State.STATE_DOUBLE_COLON) {
                        return parseError("Illegal right bracket")
                    }
                    openBrackets = false
                ...
                in '0'..'9', in 'a'..'f', in 'A'..'F' -> {
                    if (state == State.STATE_HEXDIGIT) {
                        ...
                        if (digitCount >= 4) {
                            return parseError("Too many digits in an octet")
                        }
                        ...
                        return parseError("Illegal leading single colon")
                    }
                ...
                ':' -> {
                    ...
                    // off the side!
                    if (colons >= 8) {
                        return parseError("Too many colons!")
                    }
                    ...
                    if (state == State.STATE_LBRACKET || state == State.STATE_INITIAL) {
                        ...
                        if (seenDoubleColon) {
                            return parseError("Second double colon detected")
                        }
                        ...
                        // 1:::2
                        return parseError("Illegal extra colon detected")
                    } else {
                        return parseError("Illegal colon in state $state")
                    }
    ...
    public companion object {
         ...
         */
        public fun parse(address: String): Either<ByteArray, IPAddressParseFailure> {
